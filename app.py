...
def init_db():
    with get_conn() as c:
        ...
        c.execute("""CREATE TABLE IF NOT EXISTS calc_results (
            ...
            FOREIGN KEY(measured_id) REFERENCES measured_data(id)
        )""")
        ...
        c.execute("INSERT OR IGNORE INTO sites(name,location,capacity) VALUES('Default Site','Malaysia',1.0)")
        c.commit()

def load_measured(site=None, start=None, end=None):
    q = "SELECT * FROM measured_data WHERE 1=1"
    p = []
    if site and site != "All Sites": q += " AND site_name=?"; p.append(site)
    if start: q += " AND DATE(recorded_at)>=?"; p.append(str(start))
    if end:   q += " AND DATE(recorded_at)<=?"; p.append(str(end))
...
def load_calcs(site=None, start=None, end=None):
    ...
    p = []
    if site and site != "All Sites": q += " AND site_name=?"; p.append(site)
    if start: q += " AND DATE(calc_at)>=?"; p.append(str(start))
    if end:   q += " AND DATE(calc_at)<=?"; p.append(str(end))
    q += " ORDER BY calc_at DESC"
    with get_conn() as c:
        df = pd.read_sql_query(q, c, params=p)
    if not df.empty: df["calc_at"] = pd.to_datetime(df["calc_at"])
    return df
...
def insert_measured(d):
    ...
    with get_conn() as c:
        cur = c.execute(f"INSERT INTO measured_data ({cols}) VALUES ({ph})", list(d.values()))
        c.commit(); return cur.lastrowid

def insert_calc(d):
    cols = ", ".join(d.keys()); ph = ", ".join(["?"]*len(d))
    with get_conn() as c:
        c.execute(f"INSERT INTO calc_results ({cols}) VALUES ({ph})", list(d.values()))
        c.commit()

def delete_measured(rid):
    with get_conn() as c:
        c.execute("DELETE FROM measured_data WHERE id=?", (rid,))
        c.execute("DELETE FROM calc_results WHERE measured_id=?", (rid,))
        c.commit()
...
# ═══════════════════════════════════════════════════════════════════════════════
#  SEDA CALCULATION ENGINE  (Fundamentals of Solar PV Technology, Ch 5.7–5.8)
# ═══════════════════════════════════════════════════════════════════════════════
...
with st.sidebar:
    ...
    sel_end   = st.date_input("To",   value=date.today())
    st.markdown("---")
    st.markdown('<span class="badge-online">● SYSTEM ONLINE</span>', unsafe_allow_html=True)
    st.caption(f"v3.0 · {datetime.now().strftime('%d %b %Y  %H:%M')}")
...
if page == "📊 Dashboard":
    ...
    def kpi(col, icon, val, lbl, unit):
        col.markdown(f"""<div class="kpi-card">
            ...
            <div class="kpi-unit">{unit}</div></div>""", unsafe_allow_html=True)
    ...
    if df_meas.empty:
        for c,i,v,l,u in zip([k1,k2,k3,k4,k5,k6],
            ["📋","🌞","⚡","🌡️","🧮","📐"],
            ["0","—","—","—","0","—"],
            ["Field Records","Avg Irradiance","Max Power","Avg PV Temp","Calculations","Avg Efficiency"],
            ["","W/m²","W","°C","","W·m²/W"]):
            kpi(c,i,v,l,u)
    else:
        ...
        kpi(k5,"🧮", len(df_calc),                               "Calculations",    "")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with st.form("meas_form", clear_on_submit=True):
        ...
        with c1:
            ...
            rec_time = st.time_input("🕐 Time", value=datetime.now().time())
        with c2:
    ...
    if submitted:
        ts  = datetime.combine(rec_date, rec_time).strftime("%Y-%m-%d %H:%M:%S")
    ...
    with st.form("site_form", clear_on_submit=True):
        ...
        with sc3: capacity = st.number_input("Capacity (kWp)", 0.0, step=0.1, value=1.0)
        if st.form_submit_button("➕ Add Site", use_container_width=True):
            if new_site:
                with get_conn() as conn:
                    try:
                        conn.execute("INSERT INTO sites(name,location,capacity) VALUES(?,?,?)",
                                     (new_site, location, capacity))
                        conn.commit()
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<p class="sec">🧮 SEDA CALCULATION — ROC OUTPUT ESTIMATION</p>', unsafe_allow_html=True)
    ...
    with tab_mod:
        ...
        if "last_calc" in st.session_state:
            ...
            st.markdown("---")
            st.markdown('<p class="sec">📊 CALCULATION RESULTS</p>', unsafe_allow_html=True)
            ...
            with st.expander("📐 Formula Summary"):
                st.markdown(f"""
**Power [Eq 5.7]:**
$$P_{{max}} = P_{{maxSTC}} \\times f_{{mm}} \\times f_{{degrad}} \\times f_{{temp\\_p}} \\times f_g \\times f_{{clean}} \\times f_{{unshade}}$$
= {R['p_stc_base']:.1f} × {F['f_mm']:.4f} × {F['f_degrad']:.4f} × {F['f_temp_p']:.4f} × {F['f_g']:.4f} × {F['f_clean']:.4f} × {F['f_unshade']:.4f} = **{OUT['p_max']:.3f} W**

**Degradation factor [Eq 5.8]:**  f_degrad = f_LID × f_age = {F['f_lid']:.4f} × {F['f_age']:.4f} = {F['f_degrad']:.4f}

**Irradiance factor [Eq 5.9]:**  f_g = G / 1,000 = {R['irr']:.1f} / 1000 = {F['f_g']:.4f}

**Temperature factor [Eq 5.10 / 5.12 / 5.14]:**
$$f_{{temp}} = 1 + \\left(\\frac{{\\gamma / \\beta / \\alpha}}{{100\\%}}\\right) \\times (T_{{mod}} - T_{{STC}})$$

**NOCT Model [Eq 5.4]:**  T_x = T_amb + [G × (NOCT − 20) / 800]

**Malaysian Climate Model [Eq 5.6]:**  T_x = −15.76 + 0.02G + 1.64 × T_amb
""")

    # ── ARRAY TAB ──────────────────────────────────────────────────────────────
    with tab_arr:
        st.info("To calculate for a **PV Array**, set N_s and N_p > 1 in the **MODULE / STRING** tab.")
        st.markdown("**Array Formula Extension:**")
        st.markdown("""
| Output | Formula |
|--------|---------|
...
**Example:** 2 × 30 array → set N_p = 2, N_s = 30
""")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if df_calc.empty:
        st.info("No calculation records found. Run a calculation in the 🧮 SEDA Calculator first.")
    else:
        df_c = df_calc.copy()
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_exp:
        ...
        with c1:
            ...
            if not df_meas.empty:
                buf = io.StringIO(); df_meas.to_csv(buf, index=False)
                st.download_button("⬇ Export Measured Data CSV", buf.getvalue(),
                                   "field_measured_data.csv", "text/csv")
            else:
                st.caption("No field measurement records to export.")
        with c2:
            st.markdown("**Calculation Results**")
            if not df_calc.empty:
                buf = io.StringIO(); df_calc.to_csv(buf, index=False)
                st.download_button("⬇ Export Calculation Results CSV", buf.getvalue(),
                                   "seda_calculation_results.csv", "text/csv")
            else:
                st.caption("No calculation records to export.")
